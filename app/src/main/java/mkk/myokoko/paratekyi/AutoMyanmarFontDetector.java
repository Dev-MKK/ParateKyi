...
public class AutoMyanmarFontDetector {
	...
	private void startWorking() {
		...
		mSharedPreferences = mContext.getSharedPreferences("MyanmarFontPreferences",mContext.MODE_PRIVATE);
		String status = mSharedPreferences.getString(FONT_KEY,"");
		switch (status) {
			case "": 
				startFontDetecting();
				break;
	...
	private void startFontDetecting() {
		...
		if(debugging) {
			Toast.makeText(mContext,"Font Detecting ...",Toast.LENGTH_SHORT).show();
		}

		fontTv = new TextView(mContext);

		if(mView instanceof LinearLayout){
			fontTv.setLayoutParams(getMyLinearLayoutParams());
		} else {
			fontTv.setLayoutParams(getMyRelativeLayoutParams());
		}
		fontTv.setVisibility(View.INVISIBLE);
		if(mView != null) {
		...
		*/
		...
		fontTv.setText("\u1000\u1039\u1000");
        fontTv.measure(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        int length2 = fontTv.getMeasuredWidth();
		/////////////
        ...
        try {
            ...
            while (i < length) {
                try {
                    ...
                    i++;
                    replace = replace.replaceAll(jSONObject.getString("from"), jSONObject.getString("to")).replace("null", "");
	...
	private String correctTextFromZawgyiInput(String zawgyiText) {
		return isZawGyi ? zawgyiText : zawgyiToUnicode(zawgyiText);
	}
	private String correctTextFromUnicodeInput(String unicodeText) {
		return isZawGyi ? unicodeToZawgyi(unicodeText) : unicodeText;
	}
	private void createTypeface() {
		mTypeface = Typeface.createFromAsset(mContext.getAssets(),"font/" + (isZawGyi ? "zawgyi" : "unicode") + ".ttf");
	}
	...
	public String writeZawgyi(String zawgyi) {
		return correctTextFromZawgyiInput(zawgyi);
	}
