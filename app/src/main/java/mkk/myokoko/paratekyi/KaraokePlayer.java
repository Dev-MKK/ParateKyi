...
public class KaraokePlayer implements SeekBar.OnSeekBarChangeListener {
    ...
    private int currentParateTrack = 0;
	
	public void loadKaraokeFiles(int index, ArrayList<String> list) {
        stop();
        currentParateTrack = new int[]{
			R.raw.a, R.raw.b, R.raw.c,
			R.raw.d, R.raw.e, R.raw.f,
			R.raw.g, R.raw.h, R.raw.i,
			R.raw.j, R.raw.k, R.raw.l
			}[index - 1];
        mKaraoke.feedLyrics(list, index);
        play();
    }
	
	public void seekLine(int line) {
        mKaraoke.seekLineClick(line);
    }

    public void locateKaraokeTextViews(ArrayList<TextView> list) {
        Tvs = list;
    }
	...
	private MediaPlayer getMusicPlayer() {
		...
		player = player.create(mContext, currentParateTrack);
	...
	private class Karaoke {
		...
		private void feedLyrics(ArrayList<String> lyrics, int audioDataFileIndex) {
			...
			//Toast.makeText(mContext,mLyrics.get(0),Toast.LENGTH_LONG).show();
			prepareA(0);
		...
		private void loopKaraokeA() {
			...
			if(mKaraokeLineA.hasNextRun()) {
				...
				outKaraoke(mKaraokeLineA.getKaraoke());
		...
		private void loopKaraokeB() {
			...
			if(mKaraokeLineB.hasNextRun()) {
				...
				outKaraoke(mKaraokeLineB.getKaraoke());
		...
		private void outKaraoke(KaraokeData karaokeData) {
			
            Tvs.get(karaokeLineIndex).setText(karaokeData.karaoke, TextView.BufferType.SPANNABLE);
            //Tvs.get(karaokeLineIndex).append(karaokeData.nonKaraoke);
        }
		...
		private void stop() {
			KhandlerA.removeCallbacks((Runnable)this.KrunnableA);
            KhandlerB.removeCallbacks((Runnable)this.KrunnableB);
            lineStartA = -1;
            lineEndA = -1;
            lineStartB = -1;
            lineEndB = -1;
		...
		private void seekLineClick(int n) {
            stop();
            Iterator iterator = mLyrics.iterator();
            int n2 = 0;
            do {
                if (!iterator.hasNext()) {
                    karaokeLineIndex = n - 1;
                    prepareA(n);
                    player.seekTo(-350 + mAudio[n][0]);
                    player.start();
                    return;
                }
                String string2 = (String)iterator.next();
                (Tvs.get(n2)).setText(string2.replace("$",""));
                ++n2;
            } while (true);
        }
